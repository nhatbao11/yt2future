...
export default function SignupPageClient({ errorMessage, successMessage }: SignupPageClientProps) {
  ...
  const openPdf = (url: string, title: string) => {
    if (shouldOpenPdfExternally()) {
      window.open(getPdfViewerSrc(url), '_blank', 'noopener,noreferrer');
      return;
    }
    ...
    <>
      <div className="relative z-10 bg-white p-6 md:p-10 rounded-lg shadow-2xl w-full max-w-120 border border-slate-100 transition-all text-left">
          ...
          {successMessage && (
            <div className="mt-6 p-4 bg-green-50 border-l-4 border-green-500 text-green-700 text-[11px] font-bold text-left animate-in fade-in slide-in-from-top-2">
              <span className="flex items-center gap-2">
                ...
                <span className="ml-auto animate-pulse italic font-medium opacity-70">
                  (Chuyển hướng sau 2s...)
                </span>
              </span>
            </div>
        ...
        >
          ...
          <div className="flex items-start gap-2 py-1">
            <input
              ...
              required
              id="terms"
              className="w-4 h-4 mt-0.5 cursor-pointer accent-yellow-600"
            />
            <label
              htmlFor="terms"
              className="text-[11px] text-slate-500 leading-tight cursor-pointer select-none"
            >
              {tAuth('agree')}{' '}
              <button
                type="button"
        ...
        <p className="text-[13px] mt-8 text-slate-500 text-center font-medium">
          ...
          <Link
            href="/signin"
            className="font-bold text-yellow-600 hover:text-yellow-700 underline underline-offset-4 cursor-pointer transition-colors"
          >
            {t('signInLink')}
          </Link>
        </p>
