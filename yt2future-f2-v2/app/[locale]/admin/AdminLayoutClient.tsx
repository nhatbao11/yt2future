            ...
            <nav className="bg-white border border-gray-200 rounded-lg shadow-sm p-2 flex lg:flex-col overflow-x-auto lg:overflow-visible no-scrollbar gap-1">
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isActive && (
                      <ChevronRight size={14} className="ml-auto hidden lg:block text-yellow-500" />
                    )}
                  </Link>
                );
              })}
              <div className="hidden lg:block border-t border-gray-200 mt-4 pt-4 px-2 pb-2">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-md text-red-600 hover:bg-red-50 transition font-semibold text-xs uppercase tracking-wide"
                >
                  <LogOut size={18} />
                  <span>{t('signOut')}</span>
                </button>
