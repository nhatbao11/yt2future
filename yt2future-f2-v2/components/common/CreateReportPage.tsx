...
export default function CreateReportPage({ onClose, initialData }: CreateReportProps) {
  ...
  return (
    <div
      className={`${onClose ? 'w-full' : 'max-w-4xl mx-auto py-12'} animate-in fade-in duration-500 px-4`}
    >
      {!onClose && (
        <button
          onClick={() => router.back()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <select
                  required
                  className="w-full border border-slate-300 rounded-lg p-3.5 focus:border-[#001a41] outline-none bg-white cursor-pointer"
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <label
                  className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-10 cursor-pointer transition-all ${pdfFile ? 'bg-emerald-50 border-emerald-500' : 'bg-slate-50 border-slate-300 hover:border-[#001a41]'}`}
                >
                  <input
                    type="file"
                    accept=".pdf"
                  ...
                  />
                  {pdfFile ? (
                    <CheckCircle2 className="text-emerald-600 mb-3" size={28} />
                  ) : initialData?.pdfUrl && !pdfFile ? (
                    <CheckCircle2 className="text-slate-400 mb-3" size={28} />
                  ) : (
                    <FileText className="text-slate-300 mb-3" size={28} />
                  )}
              ...
              <div className="space-y-2">
                ...
                <label
                  className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-10 cursor-pointer transition-all ${thumbnailFile ? 'bg-emerald-50 border-emerald-500' : 'bg-slate-50 border-slate-300 hover:border-[#001a41]'}`}
                >
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  {thumbnailFile ? (
                    ...
                    <ImageIcon className="text-slate-300 mb-3" size={28} />
                  )}
                  <span className="text-[11px] font-bold text-center truncate w-full px-2 italic text-slate-500">
                    {thumbnailFile
                      ? thumbnailFile.name
